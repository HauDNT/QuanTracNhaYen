...
// Trạng thái của động cơ (chạy/không chạy) dùng để cập nhật theo L298N và WebServer:
...
// Hàm điều khiển động cơ từ Webserver:
void controlEngine(String action)
{
    if (action == "start" && engineRun == false)
    {
        startEngine();
    }
    else if (action == "stop" && engineRun == true)
    {
        resetEngine();
    }
};
