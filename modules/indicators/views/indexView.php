...
<div class="content w-100 p-0 d-flex flex-column">
  <div class="row g-0">
    <ul id="tab-sensor" class="nav nav-tabs border-0">
      ...
      <li class="nav-item">
        <a class="nav-link text-secondary fw-semibold border-0 <?= $active == "indicator"  ? "active" : "" ?>" href="?mod=indicators">Chỉ số</a>
      </li>
    </ul>
  </div>
  ...
  <div class="row g-0 bg-white shadow-sm flex-fill flex-column <?= $active == "sensor"  ? "rounded-end-3 rounded-bottom-3" : "rounded-3" ?>">
    <div class="row g-0 align-items-center p-3 border-2 border-bottom border-light-subtle">
      <div class="d-flex w-auto ms-auto">
        ...
        <button type="button" class="btn btn-primary ms-auto" data-bs-toggle="modal" data-bs-target="#addIndicatorModal" data-bs-whatever="@mdo">
    ...
    <div id="table" class="row g-0 mb-3 flex-fill">
      <?php if (count($list_indicators) > 0) : ?>
...
<!-- Modal add Indicator-->
<div class="modal fade" id="addIndicatorModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Thêm chỉ số</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body overflow-y-scroll pe-2">
        <div class="mb-3">
          <label for="indicator_name" class="col-form-label">Tên chỉ số</label>
          <input type="text" class="form-control" id="indicator_name">
        </div>
        <div class="mb-3">
          <label for="indicator_unit" class="col-form-label">Đơn vị</label>
          <input type="text" class="form-control" id="indicator_unit">
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary ms-auto mr-4" data-bs-dismiss="modal">Trở lại</button>
        <button type="button" class="btn btn-primary ms-2" id="add_indicator">Thêm</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal fade" id="updateIndicatorModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
