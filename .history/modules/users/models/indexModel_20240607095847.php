...
function get_list_users_by_name($name) {
    $result = db_fetch_array("SELECT *, accounts.username, roles.name FROM userinfo, accounts, roles WHERE userinfo.account_id = accounts.id AND accounts.role_id = roles.id AND accounts.username != 'unknown' AND accounts.username LIKE '%{$name}%'");
    return $result;
}
