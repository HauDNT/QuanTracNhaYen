...
{
  $select = "SELECT s.id, s.name AS sensor_name, s.position, st.name AS station_name, s.connect_status FROM sensors s JOIN stations st ON s.station_id = st.id WHERE s.name LIKE '%{$name}%'";
  if ($status != -1) {
